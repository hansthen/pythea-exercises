...
(* utility function to sort the points (in a line segment) *)
fun sort(x1:real,y1:real,x2:real,y2:real) = 
...
fun preprocess_prog(LineSegment(x1,y1,x2,y2)) =
    ...
    then Point(x1,y1)
    else LineSegment(sort(x1,y1,x2,y2))
  | preprocess_prog(Intersect(e1,e2)) = 
    Intersect(preprocess_prog(e1), preprocess_prog(e2))
  | preprocess_prog(Let(s,e1,e2)) = 
    Let(s, preprocess_prog(e1), preprocess_prog(e2))
  | preprocess_prog(Shift(dx,dy,e)) = 
    Shift(dx, dy, preprocess_prog(e))
  | preprocess_prog(x) = x

(* utility function for shifting. NOTE: is mutually recursive with eval_prog *)
fun shift(dx, dy, e, env) = 
    case e of
    NoPoints => e
    ...
    (case List.find (fn (s2,v) => s=s2) env of
       ...
       | SOME (_,v) => shift(dx, dy, v, env))
      | Let(s,e1,e2) => shift(dx, dy, eval_prog(Let(s,e1,e2), env), env)
      | Intersect(e1, e2) => shift(dx, dy, eval_prog(Intersect(e1,e2), env), env)
...
and eval_prog (e,env) =
	...
	NoPoints => e (* first 5 cases are all values, so no computation *)
      ...
      | Line _   => e
      | VerticalLine _ => e
      | LineSegment _  => preprocess_prog(e)
      | Var s => 
	(case List.find (fn (s2,v) => s=s2) env of
	     NONE => raise BadProgram("var not found: " ^ s)
	   | SOME (_,v) => v)
      ...
      | Shift(dx, dy, exp) => shift(dx, dy, exp, env)
