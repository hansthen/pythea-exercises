...
a4 = a.intersect(Line.new(ONE,FOUR))
if not (a4.is_a? NoPoints)
	puts "a4 Point intersect not working properly with Lines"
end
a5 = a.intersect(Line.new(ONE,TWO))
if not (a5.is_a? Point)
	puts "a5 Point intersect not working properly with Lines"
end
a6 = a.intersect(LineSegment.new(ONE,TWO,THREE,FOUR))
if not (a6.is_a? NoPoints)
	puts "a6 Point intersect not working properly with LineSegments"
end
a6 = a.intersect(LineSegment.new(THREE,FIVE,FOUR,FOUR))
if not (a6.is_a? Point)
	puts "a6 Point intersect not working properly with LineSegments"
end
a7 = a.intersect(VerticalLine.new(THREE))
if not (a7.is_a? Point)
	puts "a7 Point intersect not working properly with LineSegments"
end
a8 = a.intersect(VerticalLine.new(FOUR))
if not (a8.is_a? NoPoints)
	puts "a8 Point intersect not working properly with LineSegments"
end
...
d6_1 = LineSegment.new(ZERO,ONE,TWO,THREE)
d6_2 = LineSegment.new(ONE,TWO,TWO,THREE)
d6 = d6_1.intersect(d6_2)
if not ((d6.is_a? LineSegment) and d6.x1 == d6_2.x1 and d6.y2 == d6_2.y2)
	puts "d6 LineSegment intersect not working properly"
end


d7_1 = LineSegment.new(ZERO,ZERO,FOUR,FOUR)
d7_2 = LineSegment.new(ONE,ONE,FIVE,FIVE)
d7 = d7_1.intersect(d7_2)
if not ((d7.is_a? LineSegment) and d7.y1 == ONE and d7.y2 == FOUR)
	puts "d7 LineSegment intersect not working properly"
end

d8_1 = LineSegment.new(ZERO,ZERO,FOUR,FOUR)
d8_2 = LineSegment.new(ONE,TWO,FOUR,FOUR)
d8 = d8_1.intersect(d8_2)
if not ((d8.is_a? Point) and d8.x == FOUR and d8.y == FOUR)
	puts "d8 LineSegment intersect not working properly"
end
...
puts LineSegment.new(1.0,2.0,1.0,1.0).preprocess_prog().y2

puts Let.new("x",LineSegment.new(1.0,2.0,1.0,2.0),LineSegment.new(2.0,3.0,1.0,0.0)).preprocess_prog().e1
