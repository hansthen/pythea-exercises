...
(define (racketlist->mupllist rlist)
  (cond [(null? rlist) (aunit)]
        [(pair? rlist) (apair (racketlist->mupllist (car rlist))
                              (racketlist->mupllist (cdr rlist)))]
        [#t rlist]))

(define (mupllist->racketlist mlist)
  (cond [(aunit? mlist) null]
        [(apair? mlist) (cons (mupllist->racketlist (apair-e1 mlist))
...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        [(int? e) e]      ; int is a value - return the value...
        ...
        [(mlet? e)
         (let ([ext-env (cons (cons (mlet-var e) 
        ...
        [(call? e)
         (let ([clos (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? clos)
               (let* ([s (fun-formal (closure-fun clos))]
                      [ext-env (if (fun-nameopt (closure-fun clos))
                                   (cons (cons (fun-nameopt (closure-fun clos)) clos)
                                         (cons (cons s arg)
                                               (closure-env clos)))
                                   (cons (cons s arg)
                                         (closure-env clos)))])                      
                 (eval-under-env (fun-body (closure-fun clos)) ext-env))
