...
la  $a0 Base_protObj      # get the address of the prototype
jal Object.copy                          # make a copy
jal Base_init              # and initialize it
    sw  $a0 0($fp) # and store the result in the activation record
# prepare static method call
# total # of arguments i 0
    lw    $a0 0($fp) #   local var: me
    la    $t1 Base_dispTab
    lw    $t1 28($t1)     #   @Base.identify
    jal   $t1 # 
la  $a0 Derived_protObj      # get the address of the prototype
jal Object.copy                          # make a copy
...
# prepare static method call
    ...
    sw    $a0 0($sp)
    addiu $sp $sp -4
# total # of arguments i 1
    move  $a0 $s0
    la    $t1 Base_dispTab
    lw    $t1 12($t1)     #   @Base.out_string
    jal   $t1 # 
# prepare static method call
    lw    $a0 12($fp) #   local var: s
    sw    $a0 0($sp)
    addiu $sp $sp -4
# total # of arguments i 1
    move  $a0 $s0
