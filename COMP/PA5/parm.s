...
# param 12($fp)  y:Int
 ...
 nop # start let initializer identifier
    la $a0 int_const0 # load 0
# end let initializer
    sw  $a0 4($fp) # initialize let i
    # start of let block
 nop # start let initializer identifier
    la $a0 int_const1 # load 1
# end let initializer
    sw  $a0 0($fp) # initialize let j
    lw    $a0 16($fp) #   local var: x
...
Main.main:
 # offset = 12
    ...
    sw    $a0 0($sp)  # push arg 0 on stack
    addiu $sp $sp -4 # for call to m
    la $a0 int_const3 # load 7
