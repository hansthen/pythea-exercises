...
# FIXME: generation of object references only partically done
    ...
    addiu $sp $sp -4
    move    $a0 $s0
# jump to method out_int at 16 in dispatch table 
    lw    $t1 obj_disp($a0)
    lw    $t1 16($t1)
    jalr  $t1
# push arguments on the stack
# FIXME: generation of object references only partically done
    ...
    addiu $sp $sp -4
    move    $a0 $s0
# jump to method out_string at 12 in dispatch table 
    lw    $t1 obj_disp($a0)
    lw    $t1 12($t1)
    jalr  $t1
...
end_if1:
# start coding an if statement
    # generating an isvoid expression
# FIXME: generation of object references only partically done
    lw      $a0 24($s0)
    move  $t1 $a0
    beqz   $t1, true_branch2
false_branch2:
	la	$a0 bool_const0
    b     end_if2
true_branch2:
