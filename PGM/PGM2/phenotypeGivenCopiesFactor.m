...
%phenotypeFactor.val = alphaList;
%assignments = IndexToAssignment(1:prod(phenotypeFactor.card), phenotypeFactor.card);

%for i=1:length(assignments)
%    feature = assignments(i,1);
%    genotype1 = assignments(i,2)
%    genotype2 = assignments(i,3)
%    geneCopyFactor = childCopyGivenParentalsFactor(numAlleles, feature, geneCopyVarOne, geneCopyVarTwo)
%end

for i=1:length(assignments)
    feature = assignments(i,1);
    allele1 = assignments(i,2);
    allele2 = assignments(i,3);
    genotype = allelesToGenotypes(allele1, allele2);

    if feature == 1
        phenotypeFactor.val(i) = alphaList(genotype);
    else
        phenotypeFactor.val(i) = 1 - alphaList(genotype);
    endif
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
