...
for i=1:numPeople
    ...
    if parent1 == 0
        factor = childCopyGivenFreqsFactor(alleleFreqs, i);
        factorList(i) = factor;
        factor = childCopyGivenFreqsFactor(alleleFreqs, i + numPeople);
        factorList(i+numPeople) = childCopyGivenFreqsFactor(alleleFreqs, i + numPeople);
    else 
        factor = childCopyGivenParentalsFactor(numAlleles, i, parent1, parent1 + numPeople);
        factorList(i) = factor; 
        factor = childCopyGivenParentalsFactor(numAlleles, i + numPeople, parent2, parent2 + numPeople);
        factorList(i+numPeople) = factor;
    end
end

for i=1:numPeople
    factor = phenotypeGivenCopiesFactor(alphaList, numAlleles, i, i+numPeople, 2*numPeople+i);
    factorList(2*numPeople + i) = factor;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
