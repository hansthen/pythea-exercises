  ...
  for i = 1:size([u, v], 1)
      ...
      if u(i) > v(i)
          ...
          elseif strcmp(TransName, 'MHSwendsenWang2')
              ...
              %        numerator += edge_factor.val(index);
              %    end
              %end
              assigments = zeros(2:length(edge_factor.card));
              for index=1:length(edge_factor.card)
                  assignments(index,:) = [index,index];
              end
              indices = AssignmentToIndex(assignments, edge_factor.card);
              numerator = sum(edge_factor.val(indices));
              denominator = sum(edge_factor.val);
