...
def lin_comb_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    rows = mat2rowdict(M)
    return sum(v[r] * rows[r] for r in rows)
...
def dot_product_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    rows = mat2rowdict(M)
    return Vec(M.D[0], {r: rows[r] * r for r in rows})
...
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    cols = mat2coldict(M)
    return Vec(M.D[1], {col: v * cols[col] for col in cols})
...
def dot_prod_mat_mat_mult(A, B):
    ...
    a = mat2rowdict(A)
    b = mat2coldict(B)
    return Mat((A.D[0], B.D[1]), {(i,j): a[i] * b[j] for (i,j) in itertools.product(A.D[0], B.D[1])})
...
#solving_systems_m = Mat(({0, 1}, {0, 1}), {(0,0): -1/5, (0,1): 2/5, (1,0): 4/5, (1,1): -3/5})
