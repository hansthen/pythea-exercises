...
#enc_1001 = Vec({0,1,2,3,4,5,6}, {2:one, 3:one, 6:one})
...
def find_error(e):
    ...
    for i in s.D:
        if s[i] == one:
            error |= (1 << (2 - i))
    if error:
        return Vec(set(range(7)), {error-1: one})
    else:
...
error_syn = H * non_codeword
error_vector = find_error(error_syn)
...
def find_error_matrix(S):
    ...
    """
    s = mat2coldict(S)
    return coldict2mat({c: find_error(s[c]) for c in s})
