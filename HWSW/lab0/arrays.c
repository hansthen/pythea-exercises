...
int main(int argc, char* argv[]) {
  ...
  // testing.
  // Answer: The program will write past the end of the array. 
  fillArray(array, 10);
  ...
  // Answer: Because in C array[n] is equivalent to
  //         *array + n * sizeof(array_type)
  fillArray(&value, 1);
  ...
  // C++ programming.
  // Answer: The memory will not be deallocated.
  free(heap_array);
  ...
  FourInts* four_ints2= (FourInts*) malloc(sizeof(FourInts));
  fillArray((int*) four_ints2, 4);
  assert(four_ints2->a == 2);
  assert(four_ints2->b == 5);
  assert(four_ints2->c == 8);
  assert(four_ints2->d == 11);
  free(four_ints2);
  return 0;
}
